...
const EDGE_IDX_LEFT_OF_CENTER_N1: usize = EDGE_IDX_LEFT_OF_CENTER - 1;
...
impl<K, V> InternalNode<K, V> {
        ...
        A: Allocator,
    {
        unsafe {
            ...
            LeafNode::init(ptr::addr_of_mut!((*ptr.as_ptr()).data));
            Ok(ptr)
        }
    }
}
...
impl<K, V, Type> Copy for NodeRef<marker::Raw, K, V, Type> {}
impl<K, V, Type> Clone for NodeRef<marker::Raw, K, V, Type> {
    fn clone(&self) -> Self {
        *self
    }
}
...
impl<K, V> NodeRef<marker::Owned, K, V, marker::Internal> {
        ...
        A: Allocator,
    {
        ...
        Ok(unsafe { NodeRef::from_new_internal(new_node, child.height + 1) })
    }
...
impl<BorrowType, K, V, Type> NodeRef<BorrowType, K, V, Type> {
    ...
    pub(crate) fn reborrow(&self) -> NodeRef<marker::Immut<'_>, K, V, Type> {
        NodeRef {
            height: self.height,
            node: self.node,
            _marker: PhantomData,
    ...
    /// Temporarily takes out another, raw pointer to the same node.
    pub(crate) fn raw(&self) -> NodeRef<marker::Raw, K, V, Type> {
        NodeRef {
            height: self.height,
            node: self.node,
            _marker: PhantomData,
...
impl<BorrowType: marker::BorrowType, K, V, Type> NodeRef<BorrowType, K, V, Type> {
    ...
    pub(crate) fn first_edge(self) -> Handle<Self, marker::Edge> {
        unsafe { Handle::new_edge(self, 0) }
    }

    pub(crate) fn last_edge(self) -> Handle<Self, marker::Edge> {
        let len = self.len();
        unsafe { Handle::new_edge(self, len) }
    }

    /// Note that `self` must be nonempty.
    pub(crate) fn first_kv(self) -> Handle<Self, marker::KV> {
        let len = self.len();
        assert!(len > 0);
    ...
    /// Note that `self` must be nonempty.
    pub(crate) fn last_kv(self) -> Handle<Self, marker::KV> {
        let len = self.len();
        assert!(len > 0);
    ...
    V: 'a,
{
    ...
    pub(crate) fn keys(&self) -> &[K] {
        const unsafe fn slice_assume_init_ref<T>(slice: &[MaybeUninit<T>]) -> &[T] {
            ...
            // caller guarantees that `slice` is initialized, and `MaybeUninit`
            // is guaranteed to have the same layout as `T`. The pointer
            // obtained is valid since it refers to memory owned by `slice`
        ...
        unsafe { slice_assume_init_ref(leaf.keys.get_unchecked(..usize::from(leaf.len))) }
    }
}

impl<K, V, Type> NodeRef<marker::Raw, K, V, Type> {
    /// Exposes the leaf portion of any leaf or internal node in an immutable tree.
    fn into_leaf(self) -> *const LeafNode<K, V> {
        let ptr = Self::as_leaf_ptr(&self);
        // SAFETY: there can be no mutable references into this tree borrowed as `Raw`.
        ptr as *const _
    }
}
...
impl<'a, K, V, Type> NodeRef<marker::Mut<'a>, K, V, Type> {
    ...
    /// be reawakened later.
    pub(crate) fn dormant(&self) -> NodeRef<marker::DormantMut, K, V, Type> {
        NodeRef {
            height: self.height,
            node: self.node,
            _marker: PhantomData,
...
impl<K, V, Type> NodeRef<marker::DormantMut, K, V, Type> {
    ...
    pub(crate) unsafe fn awaken<'a>(self) -> NodeRef<marker::Mut<'a>, K, V, Type> {
        NodeRef {
            height: self.height,
            node: self.node,
            _marker: PhantomData,
...
impl<'a, K, V, Type> NodeRef<marker::ValMut<'a>, K, V, Type> {
    ...
    unsafe fn into_key_val_mut_at(self, idx: usize) -> (&'a K, &'a mut V) {
        ...
        let vals: *mut [_] = vals;
        let key = unsafe { (*(keys as *const MaybeUninit<K>).wrapping_add(idx)).assume_init_ref() };
        let val = unsafe { (*(vals as *mut MaybeUninit<V>).wrapping_add(idx)).assume_init_mut() };
        (key, val)
    }
}
...
impl<K, V, Type> NodeRef<marker::Owned, K, V, Type> {
    ...
    pub(crate) fn borrow_mut(&mut self) -> NodeRef<marker::Mut<'_>, K, V, Type> {
        NodeRef {
            height: self.height,
            node: self.node,
            _marker: PhantomData,
    ...
    pub(crate) fn borrow_valmut(&mut self) -> NodeRef<marker::ValMut<'_>, K, V, Type> {
        NodeRef {
            height: self.height,
            node: self.node,
            _marker: PhantomData,
    ...
    pub(crate) fn into_dying(self) -> NodeRef<marker::Dying, K, V, Type> {
        NodeRef {
            height: self.height,
            node: self.node,
            _marker: PhantomData,
...
impl<BorrowType, K, V> NodeRef<BorrowType, K, V, marker::Leaf> {
    ...
    pub(crate) fn forget_type(self) -> NodeRef<BorrowType, K, V, marker::LeafOrInternal> {
        NodeRef {
            height: self.height,
            node: self.node,
            _marker: PhantomData,
...
impl<BorrowType, K, V, NodeType> Handle<NodeRef<BorrowType, K, V, NodeType>, marker::KV> {
    ...
    pub(crate) unsafe fn new_kv(node: NodeRef<BorrowType, K, V, NodeType>, idx: usize) -> Self {
    ...
    pub(crate) fn left_edge(self) -> Handle<NodeRef<BorrowType, K, V, NodeType>, marker::Edge> {
        unsafe { Handle::new_edge(self.node, self.idx) }
    }

    pub(crate) fn right_edge(self) -> Handle<NodeRef<BorrowType, K, V, NodeType>, marker::Edge> {
...
impl<K, V, NodeType, HandleType> Handle<NodeRef<marker::Mut<'_>, K, V, NodeType>, HandleType> {
    ...
    pub(crate) unsafe fn reborrow_mut(
        &mut self,
...
impl<BorrowType, K, V, NodeType> Handle<NodeRef<BorrowType, K, V, NodeType>, marker::Edge> {
    ...
    /// Unsafe because the caller must ensure that `idx <= node.len()`.
    pub(crate) unsafe fn new_edge(node: NodeRef<BorrowType, K, V, NodeType>, idx: usize) -> Self {
        debug_assert!(idx <= node.len());
...
fn splitpoint(edge_idx: usize) -> (usize, LeftOrRight<usize>) {
    ...
    match edge_idx {
        0..=EDGE_IDX_LEFT_OF_CENTER_N1 => (KV_IDX_CENTER - 1, LeftOrRight::Left(edge_idx)),
    ...
    V: 'a,
{
        ...
        A: Allocator,
    {
        let (mut split, handle) = match self.insert(key, value, alloc)? {
            ...
            (None, handle) => return Ok(unsafe { handle.awaken() }),
            ...
            split = match split.left.ascend() {
                Ok(parent) => {
                    match parent.insert(split.kv.0, split.kv.1, split.right, alloc)? {
                        // SAFETY: we have finished splitting and can now re-awaken the
                        // handle to the inserted element.
                        None => return Ok(unsafe { handle.awaken() }),
                ...
                Err(root) => {
                    ...
                    // SAFETY: we have finished splitting and can now re-awaken the
                    // handle to the inserted element.
                    return Ok(unsafe { handle.awaken() });
                }
            };
        }
    }
    ...
    Handle<NodeRef<BorrowType, K, V, marker::Internal>, marker::Edge>
{
    ...
    /// `edge.descend().ascend().unwrap()` and `node.ascend().unwrap().descend()` should
    /// both, upon success, do nothing.
    pub(crate) fn descend(self) -> NodeRef<BorrowType, K, V, marker::LeafOrInternal> {
        assert!(BorrowType::TRAVERSAL_PERMIT);
...
impl<K, V, NodeType> Handle<NodeRef<marker::Raw, K, V, NodeType>, marker::KV> {
    pub(crate) fn into_kv_raw(self) -> (*const K, *const V) {
        debug_assert!(self.idx < self.node.len());
        let leaf = self.node.into_leaf();
        let k = unsafe { (*leaf).keys.get_unchecked(self.idx).assume_init_ref() };
        let v = unsafe { (*leaf).vals.get_unchecked(self.idx).assume_init_ref() };
    ...
    V: 'a,
{
    ...
    pub(crate) fn into_val_mut(self) -> &'a mut V {
        debug_assert!(self.idx < self.node.len());
    ...
    V: 'a,
{
    ...
    pub(crate) fn replace_kv(&mut self, k: K, v: V) -> (K, V) {
...
impl<K, V, NodeType> Handle<NodeRef<marker::Dying, K, V, NodeType>, marker::KV> {
    ...
    pub(crate) unsafe fn into_key_val(mut self) -> (K, V) {
        debug_assert!(self.idx < self.node.len());
    ...
    /// The node that the handle refers to must not yet have been deallocated.
    #[inline]
    pub(crate) unsafe fn drop_key_val(mut self) {
        debug_assert!(self.idx < self.node.len());
        let leaf = self.node.as_leaf_dying();
        unsafe {
...
impl<'a, K, V> Handle<NodeRef<marker::Mut<'a>, K, V, marker::Internal>, marker::KV> {
    pub(crate) fn consider_for_balancing(self) -> BalancingContext<'a, K, V> {
...
impl<'a, K, V> NodeRef<marker::Mut<'a>, K, V, marker::LeafOrInternal> {
    ...
    /// the left.
    pub(crate) fn choose_parent_kv(self) -> Result<LeftOrRight<BalancingContext<'a, K, V>>, Self> {
...
impl<'a, K, V> BalancingContext<'a, K, V> {
    pub(crate) fn left_child_len(&self) -> usize {
        self.left_child.len()
    }

    pub(crate) fn right_child_len(&self) -> usize {
        self.right_child.len()
    }

    pub(crate) fn into_left_child(self) -> NodeRef<marker::Mut<'a>, K, V, marker::LeafOrInternal> {
        self.left_child
    }

    pub(crate) fn into_right_child(self) -> NodeRef<marker::Mut<'a>, K, V, marker::LeafOrInternal> {
        self.right_child
    }
    ...
    V: 'a,
{
    ...
    pub(crate) fn bulk_steal_left(&mut self, count: usize) {
        assert!(count > 0);
        unsafe {
    ...
    pub(crate) fn bulk_steal_right(&mut self, count: usize) {
        assert!(count > 0);
        unsafe {
            let left_node = &mut self.left_child;
            let old_left_len = left_node.len();
...
impl<'a, K, V, Type> Handle<NodeRef<marker::Mut<'a>, K, V, marker::LeafOrInternal>, Type> {
    ...
    pub(crate) unsafe fn cast_to_leaf_unchecked(
        self,
...
pub(crate) enum ForceResult<Leaf, Internal> {
    Leaf(Leaf),
    Internal(Internal),
}
...
pub(crate) struct SplitResult<'a, K, V, NodeType> {
    ...
    pub(crate) left: NodeRef<marker::Mut<'a>, K, V, NodeType>,
    ...
    pub(crate) right: NodeRef<marker::Owned, K, V, NodeType>,
}

impl<'a, K, V> SplitResult<'a, K, V, marker::Leaf> {
    pub(crate) fn forget_node_type(self) -> SplitResult<'a, K, V, marker::LeafOrInternal> {
        SplitResult {
