...
impl<K, V> LazyLeafRange<marker::Dying, K, V> {
    ...
    #[inline]
    pub(crate) unsafe fn deallocating_next_unchecked<A>(
        &mut self,
        alloc: &A,
    ...
    #[inline]
    pub(crate) unsafe fn deallocating_next_back_unchecked<A>(
        &mut self,
        alloc: &A,
    ) -> Handle<NodeRef<marker::Dying, K, V, marker::LeafOrInternal>, marker::KV>
    where
    ...
    #[inline]
    pub(crate) fn deallocating_end<A>(&mut self, alloc: &A)
    where
        A: Allocator,
    {
...
impl<K, V> Handle<NodeRef<marker::Dying, K, V, marker::Leaf>, marker::Edge> {
    ...
    /// - The returned KV handle is only valid to access the key and value,
    ///   and only valid until the next call to a `deallocating_` method.
    unsafe fn deallocating_next_back<A>(
        self,
        alloc: &A,
    ) -> Option<(
        Self,
    ...
    fn deallocating_end<A>(self, alloc: &A)
    where
        A: Allocator,
    {
        let mut edge = self.forget_node_type();
...
impl<K, V> Handle<NodeRef<marker::Dying, K, V, marker::Leaf>, marker::Edge> {
    ...
    unsafe fn deallocating_next_unchecked<A>(
        &mut self,
        alloc: &A,
    ) -> Handle<NodeRef<marker::Dying, K, V, marker::LeafOrInternal>, marker::KV>
    where
    ...
    unsafe fn deallocating_next_back_unchecked<A>(
        &mut self,
        alloc: &A,
    ) -> Handle<NodeRef<marker::Dying, K, V, marker::LeafOrInternal>, marker::KV>
    where
