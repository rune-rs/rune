...
use ::rust_std::iter;
use ::rust_std::mem;
use ::rust_std::ops::Bound::{self, Excluded, Included, Unbounded};
use ::rust_std::ops::RangeBounds;
use ::rust_std::panic::{catch_unwind, AssertUnwindSafe};
...
impl<K, V> BTreeMap<K, V> {
    ...
    fn dump_keys(&self) -> rust_alloc::string::String
    where
        K: Debug,
    {
        if let Some(root) = self.root.as_ref() {
            ...
            rust_alloc::string::String::from("not yet allocated")
        }
    }
        ...
        K: Ord,
    {
        ...
        if iter.len() != 0 {
            self.root.insert(Root::new(&*self.alloc).abort()).bulk_push(
                iter,
                &mut self.length,
                &*self.alloc,
            );
...
#[should_panic(expected = "range start is greater than range end in BTree")]
#[test]
...
#[should_panic(expected = "range start and end are equal and excluded in BTree")]
#[test]
...
fn test_ord_absence() {
    ...
    fn map_debug<K: Debug>(mut map: BTreeMap<K, ()>) {
        ...
        rust_alloc::format!("{:?}", map.keys());
        rust_alloc::format!("{:?}", map.values());
        rust_alloc::format!("{:?}", map.values_mut());
        if true {
            rust_alloc::format!("{:?}", map.into_iter());
        } else if true {
            rust_alloc::format!("{:?}", map.into_keys());
        } else {
            rust_alloc::format!("{:?}", map.into_values());
        }
    }
...
fn test_try_insert() {
    ...
    assert_eq!(map.insert_or(1, 10).unwrap(), &10);
    assert_eq!(map.insert_or(2, 20).unwrap(), &20);

    if let CustomError::Custom(err) = map.try_insert_or(2, 200).unwrap_err() {
...
fn test_cursor_mut() {
    ...
    assert_eq!(
        map,
        BTreeMap::from([(0, '?'), (1, 'a'), (3, 'c'), (4, 'd'), (5, 'e'), (6, 'f')])
    );
