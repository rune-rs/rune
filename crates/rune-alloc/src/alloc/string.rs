...
use super::{Allocator, Box, Error, Global, TryClone, TryExtend, TryFromIteratorIn, Vec};
...
impl<A: Allocator> String<A> {
    ...
    ///
    /// // some invalid bytes, in a vector
    /// let sparkle_heart = rune_alloc::try_vec![0, 159, 146, 150];
    ///
    ...
    pub fn try_into_boxed_str(self) -> Result<Box<str, A>, Error> {
        ...
        Ok(unsafe { crate::alloc::str::from_boxed_utf8_unchecked(slice) })
    }
...
#[cfg(feature = "std")]
impl<A: Allocator> ::rust_std::error::Error for FromUtf8Error<A> {}
#[cfg(feature = "std")]
impl ::rust_std::error::Error for FromUtf16Error {}
...
#[cfg(feature = "std")]
impl<A: Allocator> AsRef<::rust_std::ffi::OsStr> for String<A> {
    #[inline]
    fn as_ref(&self) -> &::rust_std::ffi::OsStr {
        (**self).as_ref()
    }
}
...
impl TryFrom<::rust_alloc::string::String> for String<Global> {
    ...
    ///
    /// The result is fallibly allocated on the heap.
    fn try_from(s: ::rust_alloc::string::String) -> Result<Self, Error> {
...
impl<A: Allocator> TryFrom<String<A>> for Box<str, A> {
    ...
    ///
    /// let s1: String = String::try_from("hello world")?;
    /// let s2: Box<str> = Box::try_from(s1)?;
    /// let s3: String = String::from(s2);
    ///
    /// assert_eq!("hello world", s3);
    /// # Ok::<_, rune_alloc::Error>(())
...
impl<A: Allocator> TryExtend<char> for String<A> {
    ...
    ///
    /// ```
    /// use rune_alloc::{String, IteratorExt, TryExtend};
