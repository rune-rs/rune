...
use super::{
    Allocator, Box, Error, Global, SizedTypeProperties, TryClone, TryExtend, TryFromIteratorIn,
};
...
impl<T, A: Allocator> Vec<T, A> {
    ...
    /// use rune_alloc::{Vec, AllocError, Allocator, Global};
    ///
    ...
    ///
    /// ```
    /// use rune_alloc::{Vec, IteratorExt};
    ///
    /// let mut v = rune_alloc::try_vec![1, 2, 3];
    ...
    T: TryClone,
{
    fn try_clone(&self) -> Result<Self, Error> {
        ...
        crate::alloc::slice::to_vec(self, alloc)
    }
}
...
#[cfg(feature = "alloc")]
impl<T, A: Allocator> From<Vec<T, A>> for ::rust_alloc::vec::Vec<T> {
    /// Try to convert a [`Vec<T>`] into a std `Vec<T>`.
    ///
    /// The result is allocated on the heap.
    fn from(values: Vec<T, A>) -> Self {
        let mut vec = ::rust_alloc::vec::Vec::with_capacity(values.len());
...
impl<T> TryFrom<::rust_alloc::vec::Vec<T>> for Vec<T, Global> {
    ...
    ///
    /// The result is allocated on the heap.
    fn try_from(values: ::rust_alloc::vec::Vec<T>) -> Result<Self, Error> {
        let mut v = Self::try_with_capacity_in(values.len(), Global)?;
