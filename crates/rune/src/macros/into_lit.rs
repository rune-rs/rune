...
pub trait IntoLit {
    ...
    fn into_lit(self, ctx: &mut MacroContext<'_, '_>) -> ast::Lit;
}
    ...
    ast::Number: From<T>,
{
    fn into_lit(self, ctx: &mut MacroContext<'_, '_>) -> ast::Lit {
        ...
        let id = ctx.idx.q.storage.insert_number(self);
...
impl IntoLit for &str {
    fn into_lit(self, ctx: &mut MacroContext<'_, '_>) -> ast::Lit {
        let span = ctx.macro_span();
        let id = ctx.idx.q.storage.insert_str(self);
...
impl IntoLit for String {
    fn into_lit(self, ctx: &mut MacroContext<'_, '_>) -> ast::Lit {
        let span = ctx.macro_span();
        let id = ctx.idx.q.storage.insert_string(self);
        let source = ast::StrSource::Synthetic(id);
        ast::Lit::Str(ast::LitStr { span, source })
    }
}
...
impl IntoLit for &[u8] {
    fn into_lit(self, ctx: &mut MacroContext<'_, '_>) -> ast::Lit {
        let span = ctx.macro_span();
        let id = ctx.idx.q.storage.insert_byte_string(self);
        let source = ast::StrSource::Synthetic(id);
