...
type EncodeFn<'a> = dyn Fn(&mut MacroContext<'_, '_, '_>, &mut TokenStream) + Send + Sync + 'a;

/// Construct a token stream from a function.
pub fn quote_fn<'a, T>(f: T) -> Quote<'a>
where
    T: 'a + Fn(&mut MacroContext<'_, '_, '_>, &mut TokenStream) + Send + Sync,
{
    Quote(Box::new(f))
}
...
impl<'a> Quote<'a> {
    ...
    pub fn into_token_stream(self, cx: &mut MacroContext<'_, '_, '_>) -> TokenStream {
