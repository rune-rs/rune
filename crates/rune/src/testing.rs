...
where
    T: crate::parse::Parse + crate::macros::ToTokens + PartialEq + Eq + std::fmt::Debug,
{
    let source_id = SourceId::empty();

    let mut parser = crate::parse::Parser::new(source, source_id);
    ...
    let ast2 = crate::macros::MacroContext::test(|ctx| {
        ...
        let mut parser = crate::parse::Parser::from_token_stream(&token_stream, ctx.stream_span());
