...
use crate::runtime::{
    ConstValue, FromValue, Mut, Mutable, OwnedValue, RawRefGuard, Ref, ToValue, UnsafeToMut,
...
impl FromValue for OwnedTuple {
    fn from_value(value: Value) -> VmResult<Self> {
        match vm_try!(value.take_value()) {
            ...
            OwnedValue::Ref(value) => VmResult::expected::<Self>(value.type_info()),
        }
    }
}
...
impl FromValue for Ref<Tuple> {
    fn from_value(value: Value) -> VmResult<Self> {
        let result = match vm_try!(value.into_value_shared()) {
            ...
            ValueShared::Ref(value) => Err(value.type_info()),
        };
