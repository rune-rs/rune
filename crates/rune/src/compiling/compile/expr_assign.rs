...
impl Compile2 for ast::ExprAssign {
    fn compile2(&self, c: &mut Compiler<'_>, needs: Needs) -> CompileResult<()> {
        ...
        let supported = match &self.lhs {
            ...
            ast::Expr::Path(path) if path.rest.is_empty() => {
                self.rhs.compile2(c, Needs::Value)?;
            ...
            ast::Expr::ExprIndex(expr_index_get) => {
                ...
                expr_index_get.target.compile2(c, Needs::Value)?;
                c.scopes.decl_anon(span)?;

                expr_index_get.index.compile2(c, Needs::Value)?;
                c.scopes.decl_anon(span)?;
