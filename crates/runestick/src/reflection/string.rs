...
use crate::{
    FromValue, Mut, RawValueMutGuard, RawValueRefGuard, Ref, ReflectValueType, Shared, SharedPtr,
    StrongMut, StrongRef, ToValue, UnsafeFromValue, UnsafeToValue, Value, ValueError, ValueType,
    ValueTypeInfo,
};
...
impl UnsafeFromValue for &'_ str {
    ...
    type Guard = Option<RawValueRefGuard>;
    ...
    unsafe fn unsafe_from_value(value: Value) -> Result<(Self::Output, Self::Guard), ValueError> {
        Ok(match value {
            ...
            Value::Ptr(ptr) => {
                let ptr = ptr.downcast_ref::<String>()?;
                let (string, guard) = Ref::into_raw(ptr);
...
impl UnsafeFromValue for &'_ mut String {
    ...
    type Guard = RawValueMutGuard;

    unsafe fn unsafe_from_value(value: Value) -> Result<(Self::Output, Self::Guard), ValueError> {
        Ok(match value {
            Value::String(string) => {
                let string = string.strong_mut()?;
                let (s, guard) = StrongMut::into_raw(string);
                (s, guard.into())
            }
            Value::Ptr(ptr) => {
                let ptr = ptr.downcast_mut::<String>()?;
                let (string, guard) = Mut::into_raw(ptr);
                (string, guard.into())
            }
            actual => {
                return Err(ValueError::ExpectedString {
