...
This is also an announcement for Rune `0.7`. But this shouldn't be taken too
seriously given the experimental state of development right now. In the future
we'll be trying to bake a release roughly once every month which might or might
not include breaking changes.
...
The compiler has been redesigned to avoid a common cause of miscompilations.
This is such an extensive topic that it's received its own post for those of you
who are interested in the details. But the cliff notes is that the compiler has
been redesigned in a way to make a whole class of miscompilations much harder to
introduce.

For more, please rease [Fixing a common miscompilation in Rune].

[Fixing a common miscompilation in Rune]: https://rune-rs.github.io/posts/common-miscompilation/
